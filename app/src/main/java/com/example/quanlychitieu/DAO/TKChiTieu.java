...
public class TKChiTieu extends AppCompatActivity implements View.OnClickListener {
           ...
           if(nam_khac > nam){
               ...
               if(thang_khac >=thang){
                   if(thang_khac >thang)
                       ...
                       if(thang_khac == thang){
                           if (ngay_khac < ngay) {
                               Toast.makeText(TKChiTieu.this, "Ngày bắt đầu không hợp lệ", Toast.LENGTH_LONG).show();
                               return 0;
                           } else {
                               return 1;
                           }
    ...
    public int XuLyNGay(String y){
        int ngay, thang, nam,ngay_end, thang_end, nam_end,x,xx,xxx ;
        //date Begin
        ngay = Integer.parseInt(txtNgayBD.getText().toString().substring(0,2));
        thang = Integer.parseInt(txtNgayBD.getText().toString().substring(3,5));
        nam = Integer.parseInt(txtNgayBD.getText().toString().substring(6));
        ...
        //Date End
        ngay_end = Integer.parseInt(txtNgayKT.getText().toString().substring(0,2));
        thang_end = Integer.parseInt(txtNgayKT.getText().toString().substring(3,5));
        nam_end = Integer.parseInt(txtNgayKT.getText().toString().substring(6));

        if(xxx <= nam_end && xxx >= nam){
            if(xxx == nam_end && xxx == nam){
                if(xx ==thang_end && xx == thang ){
                    if(x <= ngay_end && x >= ngay) {
                        return 1;
                    }
                    else{
                        return 0;
    ...
    public void Duyet(){
        ...
        DataBase ddBase = new DataBase(TKChiTieu.this,"DanhSachTenChiTieu.sqlite",null,1);
            if(lanDuyet==1){
                ...
                giaTriChi=HienDangTien(b);
                giaTriThu=HienDangTien(a);
